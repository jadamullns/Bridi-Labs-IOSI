function IOSIVisStimulus(angle, cyclespersecond, f, drawmask, gratingsize)
...
% grating. In each successive frame we only draw a rectangular subregion
% ofR
...
angle_list = [0, 45, 90, 135]; %MSB: Make a list of the 8 possible stimulus orientations that should be used.
%angle_list = [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330]; %12 stimuli

angle_log = [0, 45, 90, 135];
...
num_loops = 5;  %The number of loops to run, of the whole thing, I default to 10
time_ON = 0.25;    %Time for stim ON in seconds
time_OFF =63;   %Time for stim OFF in seconds

if isempty(gratingsize)
    % By default the visible grating is 400 pixels by 400 pixels in size:
    %gratingsize = 400;
    gratingsize = monitor(1,3)+ monitor(1,3); %MSB: sets the grating size to equal the largest dimension of the screen (I think)
end
...
if isempty(angle)
    ...
    %Update JM: I changed it to 90 degrees because I just want horizontal
    %lines right now. 
end

movieDurationSecs=25;   % Abort demo after 20 seconds. BUT I NO LONGER USE THIS VARIABLE

texsize=gratingsize / 2;

Screen('Preference', 'SkipSyncTests', 2); %MSB: This forces PsychToolBox to ignore the VBL sync warning that keeps popping up...
...
try
	...
	% PsychImaging() command, so we query the true values via the
    ...
    WaitSecs(60);   %MSB: I am choosing to wait for 10 seconds to record the response to blank screen at the very start of stimulation
